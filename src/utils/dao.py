...
class DAO:
  ...
  def get_one(self, id: int) -> dict | None:
    ...
    try:
      ...
      cur.execute(f"SELECT * FROM {self._table} WHERE id = ?", (id, ))
      column_names = [description[0] for description in cur.description]
  ...
  def update(self, id: int, data: dict) -> bool:
    ...
    try:
      ...
      cur.execute(f"UPDATE {self._table} SET {columns} WHERE id = ?", (*data.values(), id, ))
      self._connection.commit()
      self._disconnect()
  ...
  def delete(self, id: int) -> bool:
    ...
    try:
      ...
      cur.execute(f"UPDATE {self._table} SET DELETED = ? WHERE id = ?", (now, id, ))
      self._connection.commit()
      self._disconnect()
